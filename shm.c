...
/*
* linux / ipc / shm.c 
* 版权(C)1992年，1993年，克里希纳巴萨布拉曼尼亚
* 布鲁诺·海布尔的许多改进/修复。
* 1994年7月由“struct vm_area_struct”取代“struct shm_desc”。
* 修正了shm_unuse())1998年8月的Andrea Arcangeli。
*
* / sysvipc/shm支持(c)1999年Dragos Acostachioaie
* BIGMEM支持，Andrea Arcangeli
* SMP线程shm，让- luc Boyard
* HIGHMEM支持，Ingo Molnar
* 制作shmmax,shmall,shmmni sysctl,Christoph Rohland
* 共享/ dev/ zero支持，Kanoj Sarcar
* 将mm的功能移动到mm / shmem。c,Christoph Rohland < cr@sap.com >
*
*/
...
/*在操作系统理论中，每当提到信号量、消息队列和共享内存，
  我们通常称之为“IPC机制”；但在Linux的具体实现中，它们通常作为一种实体存在，
  占用一定的内存，具有一定的逻辑结构并拥有一组既定的操作，
  因此在研究操作系统具体实现中，我们将其称为“IPC资源”。
*/
#include "util.h"

/*共享内存允许两个或多个进程共享一给定的存储区，因为数据不需要来回复制，
  所以是最快的一种进程间通信机制。
  共享内存可以通过mmap()映射普通文件（特殊情况下还可以采用匿名映射）机制实现，也可以通过系统V共享内存机制实现。
  应用接口和原理很简单，内部机制复杂。为了实现更安全通信，往往还与信号灯等同步机制共同使用。
  共享内存涉及到了存储管理以及文件系统等方面的知识，深入理解其内部机制需要紧紧抓住内核使用的重要数据结构。
  系统V共享内存是以文件的形式组织在特殊文件系统shm中的。
  通过shmget可以创建或获得共享内存的标识符。取得共享内存标识符后，
  要通过shmat将这个内存区映射到本进程的虚拟地址空间。

  共享内存允许一个或多个进程通过同时出现在它们的虚拟地址空间的内存通讯。
  这块虚拟内存的页面在每一个共享进程的页表中都有页表条目引用。但是不需要在所有进程的虚拟内存都有相同的地址。
  象所有的系统 V IPC 对象一样，对于共享内存区域的访问通过 key 控制，
  并进行访问权限检查。内存共享之后，就不再检查进程如何使用这块内存。
  它们必须依赖于其他机制，比如系统 V 的信号量来同步对于内存的访问。 

  每一个新创建的内存区域都用一个 shmid_ds 数据结构来表达。这些数据结构保存在 shm_segs 向量表中。 Shmid_ds数据结构描述了这个共享内存取有多大、多少个进程在使用它以及共享内存如何映射到它们的地址空间。
  由共享内存的创建者来控制对于这块内存的访问权限和它的 key 是公开或私有。
  如果有足够的权限它也可以把共享内存锁定在物理内存中。 
*/
...
/*系统V共享内存是以文件的形式组织在特殊文件系统shm中的。
  ...
  系统V共享内存原理
  系统V共享内存指的是把共享数据放在共享内存区域（IPC shared memory region），
  任何想要访问该数据的进程通过共享该内存区域来获得访问权。
  系统V共享内存通过shmget获得或创建一个IPC共享内存区域，并返回相应的标识符。
  内核在保证shmget获得或创建一个共享内存区，初始化该共享内存区相应的 shmid_kernel结构注同时，
  还将在特殊文件系统shm中，创建并打开一个同名文件，
  并在内存中建立起该文件的相应dentry及inode结构，
...
{	
	...
	/* used by in-kernel data structures 许可证,所有的IPC资源都与这样一个数据结构相关，
	   以表示其所有者、建立者等必要信息期望将其放入系统内核已达到进程间通信的最高效率
	*/
	...
	/* 存储了将被映射文件的地址。每个共享内存区对象都对应特殊文件系统shm中的一个文件，
	一般情况下，特殊文件系统shm中的文件是不能用read()、write()等方法访问的，
	当采取共享内存的方式把其中的文件映射到进程地址空间后，
	可直接采用访问内存的方式对其访问
	*/
	int			id;
...
/*当一个新的共享内存段的需要创建时，newseg()函数被调用。
它对新段的三个参数起作用：键、标志和大小。
经过验证的共享内存段的大小在SHMMIN和SHMMAX之间。
共享内存段的总数不超过SHMALL，它分配一个新的共享内存段描述符。
*/
/*
newseg创建新的键值为key，
大小为size的共享内存，
